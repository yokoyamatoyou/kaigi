...
class EventsMixin:
                ...
                ft.Container(
                    ft.Column([
                        ...
                        ft.TextField(
                            ...
                            max_lines=2,
                            data=i,
                            on_change=self._on_persona_changed,
                            dense=True,
                        ),
    ...
    def _on_remove_model_clicked(self, e: ft.ControlEvent):
        ...
        self._remove_model(index_to_remove)
        self.page.update()
    ...
    async def _on_file_picked(self, e: ft.FilePickerResultEvent):
        ...
        if e.files and e.files[0] and e.files[0].path:
            ...
            self.file_status_text.color = "green"
            ...
            if e.files and e.files[0] and not e.files[0].path:
    ...
    async def _start_meeting(self, e):
        ...
        if not moderator_name_selected:
        ...
        try:
            ...
            if not moderator_model_info:
                self._show_snack_bar(
                    f"エラー: 選択された司会モデル '{moderator_name_selected}' が参加者リストに見つかりません。"
                )
            ...
            result_from_manager = await self.meeting_manager.run_meeting(
                settings, progress_callback=self._on_progress_update
            )

            if result_from_manager:
                logger.info(
                    f"会議結果受け取り成功。要約冒頭: '{result_from_manager.final_summary[:50] if result_from_manager.final_summary else 'N/A'}'"
                )
            ...
            if self.current_meeting_result:
                ...
                logger.info(
                    f"DEBUG: current_meeting_result.conversation_log is not None: {self.current_meeting_result.conversation_log is not None}"
                )
                if self.current_meeting_result.conversation_log:
                    logger.info(
                        f"DEBUG: len(current_meeting_result.conversation_log): {len(self.current_meeting_result.conversation_log)}"
                    )
                ...
                logger.info(
                    f"DEBUG: current_meeting_result.final_summary: '{self.current_meeting_result.final_summary[:50] if self.current_meeting_result.final_summary else 'None or Empty'}'"
                )
                if self.current_meeting_result.final_summary and self.current_meeting_result.final_summary.strip():
                    can_save_result = True
                else:
                    logger.info("最終要約が空またはNoneです。結果保存ボンは無効のままです。")

                if (
                    not self.result_text.value.startswith("会議中に致命的なエラー")
                    and not self.result_text.value.startswith("会議の実行に失敗しました")
                    and can_save_result
                ):
                    self._show_snack_bar(
            ...
            logger.info(
                f"DEBUG: Save Conversation Button - Disabled: {self.save_conversation_button.disabled}, Visible: {self.save_conversation_button.visible if hasattr(self.save_conversation_button, 'visible') else 'N/A'}"
            )
            logger.info(
                f"DEBUG: Save Result Button - Disabled: {self.save_result_button.disabled}, Visible: {self.save_result_button.visible if hasattr(self.save_result_button, 'visible') else 'N/A'}"
            )
                ...
                if hasattr(ctrl.content, "controls"):
                    for item_in_col in ctrl.content.controls:
                        if isinstance(item_in_col, ft.Row) and hasattr(item_in_col, "controls"):
                            for sub_item in item_in_col.controls:
                                if hasattr(sub_item, "disabled"):
                                    sub_item.disabled = processing
                        elif hasattr(item_in_col, "disabled"):
    ...
    async def _save_conversation(self, e):
        ...
        logger.info(
            f">>> _save_conversation: hasattr(settings): {hasattr(self.current_meeting_result, 'settings')}"
        )
        if hasattr(self.current_meeting_result, "settings") and self.current_meeting_result.settings:
            logger.info(
                f">>> _save_conversation: settings.user_query: {self.current_meeting_result.settings.user_query[:50] if self.current_meeting_result.settings.user_query else 'N/A'}"
            )
        ...
        if not self.current_meeting_result.conversation_log:
            ...
            return
        logger.info(
            f">>> _save_conversation: Conversation log count: {len(self.current_meeting_result.conversation_log)}"
        )
        ...
        try:
            if (
                hasattr(self.current_meeting_result, "settings")
                and self.current_meeting_result.settings
                and hasattr(self.current_meeting_result.settings, "user_query")
                and self.current_meeting_result.settings.user_query
            ):
            ...
            logger.error(
                f">>> _save_conversation: Error getting query for filename: {ex_fn}", exc_info=True
            )
        ...
        if hasattr(self.current_meeting_result, "settings") and self.current_meeting_result.settings:
            ...
            if hasattr(settings_data, "user_query"):
                lines.append(f"- **質問/指示:** {settings_data.user_query}")
            if hasattr(settings_data, "document_path") and settings_data.document_path:
                lines.append(f"- **資料ファイル:** {Path(settings_data.document_path).name}")
        else:
            logger.warning(">>> _save_conversation: MeetingResult.settings is missing.")
        if hasattr(self.current_meeting_result, "participants_count"):
            lines.append(f"- **参加者数:** {self.current_meeting_result.participants_count}")
        if hasattr(self.current_meeting_result, "duration_seconds"):
            lines.append(
                f"- **所要時間:** {format_duration(self.current_meeting_result.duration_seconds)}"
            )
        if hasattr(self.current_meeting_result, "total_tokens_used"):
            lines.append(
                f"- **使用トークン:** {self.current_meeting_result.total_tokens_used}"
            )
        lines.append("\n## 会話内容\n")
        for entry in self.current_meeting_result.conversation_log:
            ...
            lines.append(
                f"### [{format_timestamp(timestamp)}] Round {round_num} - {speaker} (ペルソナ: {persona}, モデル: {model_name})\n"
            )
        ...
        try:
            ...
            logger.info(
                f">>> _save_conversation: Successfully saved conversation to: {file_path}"
            )
        except Exception as ex:
    ...
    async def _save_result(self, e):
        ...
        logger.info(
            f">>> _save_result: hasattr(settings): {hasattr(self.current_meeting_result, 'settings')}"
        )

        final_summary_to_save = ""
        if (
            hasattr(self.current_meeting_result, "final_summary")
            and self.current_meeting_result.final_summary
        ):
            final_summary_to_save = self.current_meeting_result.final_summary.strip()

        logger.info(
            f">>> _save_result: Final summary to save (first 50 chars): '{final_summary_to_save[:50]}'"
        )
        ...
        try:
            ...
            logger.error(
                f">>> _save_result: Error getting query for filename: {ex_fn}", exc_info=True
            )
        ...
        if hasattr(self.current_meeting_result, "settings") and self.current_meeting_result.settings:
            ...
            logger.warning(">>> _save_result: MeetingResult.settings is missing.")
        if hasattr(self.current_meeting_result, "duration_seconds"):
            lines.append(
                f"- **所要時間:** {format_duration(self.current_meeting_result.duration_seconds)}"
            )
